...
[
  ...
  {{ "surfaceUpdate": {{
    "surfaceId": "default",
    "components": [
      {{ "id": "root-column", "component": {{ "Column": {{ "children": {{ "explicitList": ["title-heading", "rows-list"] }} }} }} }},
      {{ "id": "title-heading", "component": {{ "Text": {{ "usageHint": "h1", "text": {{ "literalString": "Top Restaurants" }} }} }} }},
      {{ "id": "rows-list", "component": {{ "List": {{ "direction": "vertical", "children": {{ "template": {{ "componentId": "row-template", "dataBinding": "/rows" }} }} }} }} }},
      {{ "id": "row-template", "component": {{ "Row": {{ "children": {{ "template": {{ "componentId": "item-card-template", "dataBinding": "items" }} }} }} }} }},
      {{ "id": "item-card-template", "weight": 1, "component": {{ "Card": {{ "child": "card-layout" }} }} }},
      {{ "id": "card-layout", "component": {{ "Row": {{ "children": {{ "explicitList": ["template-image", "card-details"] }} }} }} }},
      {{ "id": "template-image", "weight": 1, "component": {{ "Image": {{ "url": {{ "path": "imageUrl" }} }} }} }},
      {{ "id": "card-details", "weight": 2, "component": {{ "Column": {{ "children": {{ "explicitList": ["template-name", "template-rating", "template-detail", "template-link", "template-book-button"] }} }} }} }},
      {{ "id": "template-name", "component": {{ "Text": {{ "usageHint": "h3", "text": {{ "path": "name" }} }} }} }},
  ...
  {{ "dataModelUpdate": {{
    ...
    "contents": [
      ...
      ] }} // Populate this with restaurant data grouped into rows of 2
    ]
  }} }}
]
...
def get_ui_prompt(base_url: str, examples: str) -> str:
    ...
    -   If the number of restaurants is 5 or fewer, you MUST use the `SINGLE_COLUMN_LIST_EXAMPLE` template.
        -   Structure the data as a flat `items` valueMap containing each restaurant.
    -   If the number of restaurants is more than 5, you MUST use the `TWO_COLUMN_LIST_EXAMPLE` template.
        -   Structure the data as `rows`, where each row contains an `items` valueMap with exactly 2 restaurants.
        -   Group restaurants into pairs: row1 has items 1-2, row2 has items 3-4, etc.
        -   If there's an odd number of restaurants, the last row will have only 1 item.
